...
- **選択したテキストのみ**が翻訳のために外部サービス（Google/OpenAI）へ送信されます。ファイル名や周辺メタデータは送信されません。
- 送信はホバー時（デバウンス後）、クリップボード翻訳コマンド実行時、LLM 言語検出（有効時）で行われます。
- OpenAI の system-role 判定のために短いテストリクエストを送信することがあります（ユーザー選択テキストは含みません）。
- API キーはログに出力しない方針です（設定に保存）。
